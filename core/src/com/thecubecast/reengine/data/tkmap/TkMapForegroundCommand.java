...
public class TkMapForegroundCommand implements TkMapCommand {
    ...
    public TkMapForegroundCommand(int x, int y, int ID, EditorState.BrushSizes size, TkMap map) {
        this.x = x;
        this.y = y;
        this.ID = ID;
        this.map = map;
    ...
    public void UnExecute() {
        for (int i = 0; i < Tiles.size(); i++) {
            map.getForeground()[(int)Tiles.get(i).x][(int)Tiles.get(i).y] = (int)Tiles.get(i).z;
        }
    }
}
